...
class S2PrdIdInfo:
    ...
    def __init__(self, s2_prd_id) -> None:
        ...
        if len(elt_prd_id) == 7:
            ...
            raise ValueError(
                "Sentinel 2 product id not provides the 7 keys values requested!"
            )
    ...
    def product_discriminator(self, value):
        if len(value) == 15:
            ...
            raise ValueError(
                "Length of product discriminator different than 15 is not possible!"
            )
    ...
    def relative_orbit_number(self, value):
        if len(value) == 3:
            if int(value) >= 1 and int(value) <= 143:
                ...
                raise ValueError(
                    f"Relative orbit number {value} is not possible ([001, R143])!"
                )
        else:
            raise ValueError(
                "Length of relative orbit number different than 3 is not possible!",
                value,
            )
    ...
    def datatake_sensing_start_time(self, value):
        self._datatake_sensing_start_time = datetime.strptime(
...
if __name__ == "__main__":
    print(
        S2PrdIdInfo("S2B_MSIL1C_20210714T235249_N0301_R130_T57KUR_20210715T005654.SAFE")
    )
    print(
        S2PrdIdInfo("S2B_MSIL2A_20210714T131719_N0301_R124_T28WDB_20210714T160455.SAFE")
    )
    print(
        S2PrdIdInfo.is_valid(
            "S2B_MSIL2_20210714T131719_N0301_R124_T28WDB_20210714T160455.SAFE"
        )
    )
    print(
        S2PrdIdInfo.is_valid(
            "S2B_MSIL1C_20210714T235249_N0301_R200_T57KUR_20210715T005654.SAFE"
        )
    )
