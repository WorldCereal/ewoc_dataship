...
class AWSEOBucket(EOBucket):
    _SUPPORTED_BUCKET=['sentinel-s1-l1c',
                       'sentinel-s2-l1c',
                       'sentinel-s2-l2a',
                       'sentinel-cogs',
                       'usgs-landsat',
    ...
    def __init__(self, arn_suffix) -> None:

        if os.getenv('AWS_ACCESS_KEY_ID') is None or os.getenv('AWS_SECRET_ACCESS_KEY') is None:
            logger.warning('AWS S3 crendentials not set by env variables, \
...
class AWSS1Bucket(AWSEOBucket):
    ...
    def download_prd(self, prd_id: str, out_dirpath_root: Path=Path(gettempdir())) -> None:
...
class AWSS2Bucket(AWSEOBucket):
    ...
    def _download_s2_prd(self, prd_id:str, out_dirpath_root:Path,
...
class AWSS2L2ABucket(AWSS2Bucket):
    ...
    def download_prd(self, prd_id: str, out_dirpath_root: Path=Path(gettempdir()), l2a_mask_only:bool=False) -> None:
        return super()._download_s2_prd(prd_id, out_dirpath_root, l2_mask_only=l2a_mask_only)
...
class AWSS2L2ACOGSBucket(AWSS2Bucket):
    ...
    def download_prd(self, prd_id: str, out_dirpath_root: Path=Path(gettempdir()), l2a_mask_only:bool=False) -> None:
        return super()._download_s2_prd(prd_id, out_dirpath_root, l2_mask_only=l2a_mask_only, l2a_cogs=True)
...
class AWSCopDEMBucket(AWSEOBucket):
    ...
    def download_tiles(self, copdem_tile_ids:List[str], out_dirpath:Path=Path(gettempdir()),
                              ) -> None:
        ...
        for copdem_tile_id in copdem_tile_ids:
            ...
            copdem_tile_id_aws = self._copdem_prefix + \
