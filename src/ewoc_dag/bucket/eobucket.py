...
class EOBucket():
    ...
    def _download_prd(self, prd_prefix:str, out_dirpath:Path,
    ...
    def _upload_file(self, filepath: Path, key: str)-> bool:
        ...
        try:
            logger.info('Try to upload %s to %s', filepath, 's3://'+self._bucket_name+'/' + key)
    ...
    def _upload_prd(self, prd_dirpath: Path, object_prefix:str, file_suffix:str='.tif'):
        ...
        logger.info('Uploaded content of %s with %s suffix (%s file(s) with total size of %s) to %s',
            prd_dirpath,
            file_suffix,
            nb_filepath,
            upload_object_size,
